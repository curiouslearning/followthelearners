...
async function assignExpiringLearners() {
  ...
  try {
    [priorityQueue, learnerSnap] = await Promise.all(getPriorityQueue(), getLearnerQueue());
  } catch(err) {
  ...
  if(priorityQueue.length === 0) {
    console.warn('There are no users to process');
  ...
  let learnerQueue = prioritizeLearnerQueue.sort(
...
async function sweepExpiredLearners() {
  ...
  snap.forEach((doc)=>{
    ...
    batch.set(poolRef.doc(doc.id), data, {merge: true});
  });
  return batch.commit();
}
